	...
	static class Solution {
	    public String solution(String new_id) {
	    	...
	    	for(int i=0;i<answer.length();i++) {
	    		if(c=='.' && answer.charAt(i)=='.') continue; // 이전글자도 .이고 현재도 .이면 통과
	    		t += answer.charAt(i); // 붙이고
	    		c = answer.charAt(i);  // 이전글자 변경
	    	}
	    	answer = t;
	    	System.out.println(answer);
	    	// 4단계 new_id에서 마침표(.)가 처음이나 끝에 위치한다면 제거합니다.
	    	if(answer.charAt(0)=='.') answer = answer.substring(1); // 첫글자가 .이면 버린다.
	    	if(!answer.equals("") && answer.charAt(answer.length()-1)=='.') answer = answer.substring(0, answer.length()-1); // 마지막글자 버림
	    	System.out.println(answer);
	    	// 5단계 new_id가 빈 문자열이라면, new_id에 "a"를 대입합니다.
