...
public class Ex03 {
	public static void main(String[] args) {
		...
		System.out.println(example(100, 200));
		System.out.println(example(201, 210));
		System.out.println(example(900, 1000));
	}

	public static int example(int n, int m) {
		int maxCycle = -999;
		for(int i=n;i<=m;i++) {
			if(maxCycle<getCycle(i)) {
				maxCycle = getCycle(i);
			}
		}
		return maxCycle;
	...
	public static int getCycle(int n) {
		// n이 짝수면 2로 나누고, 홀수면 3을 곱한 다음 1을 더한다.
		int cycle=1;
		while(n!=1) {
			//System.out.print(n + " ");
			n = n%2==0 ? n/2 : n*3+1;
