		...
		public int[] solution(int n, int[] stages) {
			...
			for (int i = 1; i <= n; i++) {
				...
				if (frequency.containsKey(i)) {
					failureRate.put(i, (double) (frequency.get(i)) / numberOfUsers); // 스테이지번호/ 도전자수
					numberOfUsers -= frequency.get(i); // 인원수 감소
				} else {
			...
			// 결과는 실패율이 아니라 스테이지 번호가 필요하다.
			Iterator<Map.Entry<Integer, Double>> it = list.iterator();
			for(int i=0;it.hasNext();i++) {
				answer[i] = it.next().getKey(); // 결과는 키값
			}
