...
3번 폰켓몬 한 마리, 2번 폰켓몬 한 마리, 4번 폰켓몬 한 마리를 고르면 되며, 
따라서 3을 return 합니다.
...
public class Ex02 {
	...
	static class Solution {
	    public int solution(int[] nums) {
	        int length = nums.length/2; // 최대로 뽑을 수 있는 포켓몬의 개수
	        Set<Integer> set = new HashSet<>(IntStream.of(nums).boxed().collect(Collectors.toList())); // 셑에 넣어 중복 제거
	        System.out.println(set);
	        return length>set.size() ? set.size():length; // 중복을 제거한 개수보다 위에수 구한 개수가 크면 종류의 개수를 아니면 최대개수를 리턴
	    }
	}
}
