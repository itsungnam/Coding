...
일단 어떤 수를 받아서 그 수를 뒤집은 다음 뒤집어진 수를 원래의 수에 더하는 과정을 뒤집어서 더하기라고 
부르자. 
...
어떤 수가 주어졌을 때 회문이 있으면 출력하고, 그 회문을 찾기까지 뒤집어서 더하기를 반복한 횟수를 출력하는 
프로그램을 만들어야 한다.
...
public class Ex02 {
	...
	public static  void example(int n) {
		int count = 0;
		while(true) {
			StringBuffer sb = new StringBuffer(n+""); // 문자로 만든다.
			sb.reverse(); // 두집는다.
			if((n+"").equals(sb.toString())) { // 뒤집은값과 같으면 종료
				break;
			}
			n = n + Integer.parseInt(sb.toString()); // 원래수 와 뒤집은수를 더한값을 다시 취한다. 
			count++;
		}
