...
배열을 소팅하는 것은 초보 프로그래머에게 유명한 문제이며 전문적인 
프로그래밍(데이터베이스, 라이브러리등)에서도 매우 중요하게 다루어 진다.

소팅은 비교에 기반을 두어 배열을 재 정렬하는 방법이다. 다음의 배열을 생각해 보자.
a = [3, 1, 4, 1, 5, 9, 2, 6]
우리는 이 배열을 크기 순서대로 정렬하고 싶다. 
(좌측의 요소는 우측의 요소보다 작거나 같아야 한다.)
수학적으로 표현하자면 다음과 같다.
i < j, a[i] <= a[j]
...
i와 j라는 인덱스를 가진 두 개의 값을 서로 바꾸기(Swap) 위해서는 몇가지 방법이 있는데 
임시 변수를 사용하는 예는 다음과 같다:
t = a[i]
a[i] = a[j]
a[j] = t
...
public class Ex02 {
	public static void main(String[] args) {
		example(8, new int[]{3,1,4,1,5,9,2,6});
	}
	public static  void example(int count, int[] ar) {
		System.out.println("정렬전 : " + Arrays.toString(ar));
		int i,cnt=0;
		for(i=0;i<count-1;i++) { // n-1회
			boolean flag = true; // 정렬되었다고 가정
			for(int j=0;j<count-1-i;j++) { 
				if(ar[j]>ar[j+1]) { // 바로 인접한 값과 비교
					cnt++; // 교환 횟수
					int t = ar[j];
					ar[j] = ar[j+1];
					ar[j+1] = t;
