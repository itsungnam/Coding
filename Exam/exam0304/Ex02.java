...
UVA 10591 문제이고, 이스트소프트 개발직군 샘플문제로 공개된 자료를 가져왔습니다. 
채점기준을 준수한다면 난이도는 좀 더 상승될 것 같습니다.
채점기준에서 속도가 중요하다 했는데, 대부분의 수들이 길지 않은 싸이클로 반복하다보니 처리시간은 순식간입니다. 
수십, 수백만개의 Case에 대해 처리시간을 체크해봐야 차이가 날것 같습니다.
*/
...
public class Ex02 {
	...
	public static void example(int n) {
		long startTime = System.nanoTime(); // 시작 시작
		boolean flag = false; // 해피넘버가 아니라고 가정
		int temp = n; // n을 피신
		do {
			int sum = 0; // 제곱의 합
			while(temp>0) {
		...
		}while(temp!=n); // 합이 처음 숫자가 아닌동안 반복
		System.out.println();
		System.out.println( n + "is a  " + (flag ? "Happy":"Unhappy") + " number.");
